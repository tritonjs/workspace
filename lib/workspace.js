...
class Workspace {
  ...
  heartbeat(username, done) {
    REDIS_STATUS.hgetall('status:'+username)
    .then(result => {
      debug('heatbeat:result', result);

      result.checkin = Date.now().toString();
      result.max     = 600000; //
      result.online  = true;
    })
  }
   ...
   **/
  updateImage() {
    docker.pull(CONFIG.docker.image, function (err, stream) {
      if(err) {
        return debug('updateImage', 'failed', err);
      }
    });
  }
}
...
sub.on('message', (channel, message) => {
  ...
  if(channel === 'WorkspaceUpdate') {
    if(p.id === process.env.HOST) return debug('sub:update', 'ignoring, already did ('+p.id+' == '+process.env.HOST+' )')

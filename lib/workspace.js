...
sub.on('message', (channel, message) => {
  ...
  if(!p.username) return debug('WorkspaceDelete', 'invalid username', p.username);

  auth.getUserWorkspace(p.username)
  .then(cont => {
    let cont_id = cont.old;

    if(p.inactive) {
      debug('container:select', 'WARNING: targetting (in?)active container!')
      cont_id = cont.id;
    }

    debug('container:select', 'ID is', cont_id);

    let container = docker.getContainer(cont_id);
    container.stop(err => {
      if(err) {
      ...
      container.remove(err => {
        ...
        debug('container:remove', 'removed', cont_id);
      })

      debug('container:stop', 'stopped', cont_id);
    })
  })
  .catch(err => {
    if(err) {
...
let gc = () => {
  ...
  stream.on('end', () => {
    pipe.exec((err, res) => {
      res.forEach(wrkspce => {
        wrkspce = wrkspce[1];
        debug('scheduled:heartbeat:process', wrkspce);
        let now = Date.now();
        let lastcheckin = now - wrkspce.checkin;

        // check data.
        if(!wrkspce.username) {
          return debug('scheduled:heartbeat:validate', 'fail invalid username');
        }

        wrkspce.max     = parseInt(wrkspce.max);
        wrkspce.checkin = parseInt(wrkspce.checkin);

        if(wrkspce.online === false || wrkspce.online === 'false') return;

        if(lastcheckin < 0) {
          debug('scheduled:heartbeat:check', 'in the future?')
          return;
        } else if(lastcheckin < wrkspce.max) {
          return;
        }

        debug('scheduled:heartbeat:check', wrkspce.username, 'inactive -- remove.');
        pub.publish('WorkspaceDelete', JSON.stringify({
          username: wrkspce.username,
          inactive: true
        }));
  ...
  let NEXT = Math.floor(Math.random() * 600000);
  debug('scheduled:heartbeat', 'running in', NEXT);
  setTimeout(gc, NEXT);
};

/**
 * Instance the class, and start workspace gc.
 *
 * @param {Object} d - database object from db.js
 * @param {Object} a - auth object from auth.js
 * @param {Object} r - redis from redis.js
 *
 * @returns {Object} workspace instanced.
 */
module.exports = (d, a, r) => {
  let NEXT = Math.floor(Math.random() * 300000);
  debug('scheduled:heartbeat', 'running in', NEXT);
  setTimeout(gc, NEXT)
